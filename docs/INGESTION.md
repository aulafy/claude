...
The current quality gate is in `lib/brain/evals.ts`. It keeps a small, versioned golden set of real fixture queries and reports recall@5 and mean reciprocal rank. It also checks that tutor citations match retrieval provenance and that abstention remains honest. These evals are intentionally small and will grow with canonical content; a 100% score here does not claim production-scale semantic quality.
