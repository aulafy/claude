...
M0.7 provides `lib/brain/learning-loop.ts` for the Local AI vertical slice. It derives an ordered lesson path from stable content IDs and prerequisites, requires completion before project evidence, distinguishes submitted from verified evidence, and calculates progress from events rather than fake percentages. M0.7/M0.8 also provide authenticated persistence for lesson progress and evidence, server-side prerequisite enforcement, a private learner route, and role-gated evidence review with approval or rejection. The domain loop remains deterministic and in-memory; Supabase is the persistence boundary.
