...
import EvidenceReviewQueue from "@/components/brain/EvidenceReviewQueue";
...
export default async function Moderacion() {
  ...
  let evidence: Array<{ id: string; created_at: string; payload: Record<string, unknown> | null }> = [];
  if (isSupabaseConfigured()) {
    const db = await createSupabaseServerClient();
    const result = await db.from("aulafy_evidence").select("id,created_at,payload").eq("status", "submitted").order("created_at", { ascending: true });
    evidence = (result.data ?? []) as typeof evidence;
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12 sm:py-14">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        {member.role === "admin" && <Link href="/admin/aprendizaje" className="aula-button aula-button-secondary min-h-9 px-4 py-2 text-sm"><Icon name="chart" /> Aprendizaje</Link>}
      </div>
      ...
      <EvidenceReviewQueue initialEvidence={evidence} />
    </div>
  );
}
