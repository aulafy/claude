...
export default function BrainProgressPanel({ lessons, authenticated, evidence: submittedEvidence, english = false }: { lessons: Lesson[]; authenticated: boolean; evidence: Evidence[]; english?: boolean }) {
  ...
  if (!authenticated) return <div className="grid gap-6 lg:grid-cols-[1fr_0.8fr]"><section className="aula-panel p-5"><div className="flex items-baseline justify-between gap-4"><h2 className="font-display text-2xl font-bold text-white">{english ? "Learning path" : "Ruta de aprendizaje"}</h2><span className="text-sm text-zinc-400">{lessons.length} {english ? "lessons" : "lecciones"}</span></div><p className="mt-2 text-sm text-zinc-400">{english ? "Read the open material first. Sign in only when you want to save progress and submit evidence." : "Lee primero el material abierto. Entra solo si quieres guardar el progreso y enviar evidencias."}</p><ol className="mt-5 grid gap-3">{lessons.map((lesson, index) => <li key={lesson.id} className="flex items-center gap-3 rounded-lg border border-zinc-800 p-3"><span className="text-xs text-zinc-500">{index + 1}</span><span className="text-zinc-200">{lesson.title}</span></li>)}</ol></section><section className="aula-panel p-5"><h2 className="font-display text-2xl font-bold text-white">{english ? "Save your work" : "Guarda tu trabajo"}</h2><p className="mt-2 text-sm text-zinc-400">{english ? "Your account is optional. It is used only for progress, project evidence and review." : "La cuenta es opcional. Solo se usa para el progreso, las evidencias del proyecto y su revisión."}</p><Link className="aula-button aula-button-primary mt-4 inline-flex" href={english ? "/acceso?next=%2Fen%2Fbrain" : "/acceso?next=%2Fbrain"}>{english ? "Sign in" : "Entrar"}</Link></section></div>;
