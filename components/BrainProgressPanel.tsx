...
type Evidence = { id: string; status: string; created_at: string };
export default function BrainProgressPanel({ lessons, authenticated, evidence: submittedEvidence }: { lessons: Lesson[]; authenticated: boolean; evidence: Evidence[] }) {
  ...
  const submitEvidence = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        setEvidenceItems((current) => [{ id: result.evidenceId, status: "submitted", created_at: new Date().toISOString() }, ...current]);
        setFormEvidence({ model: "", task: "", result: "" });
        setMessage("Evidencia enviada para revisión.");
      } else setMessage(result.message ?? "No se pudo enviar la evidencia.");
    });
  };
  ...
  const evidenceLabel = (status: string) => status === "verified" ? "Verificada" : status === "rejected" ? "Rechazada: corrige y vuelve a enviar" : "Pendiente de revisión";
