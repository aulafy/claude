...
import { verifyEvidence } from "@/app/brain/actions";
...
export default function EvidenceReviewQueue({ initialEvidence }: { initialEvidence: Evidence[] }) {
  ...
  const review = (id: string) => startTransition(async () => {
    const result = await verifyEvidence(id);
    if (result.ok) setItems((current) => current.filter((item) => item.id !== id));
    setMessage(result.ok ? "Evidencia verificada." : result.message ?? "No se pudo verificar.");
  });
  ...
  return (
    <section className="aula-panel mt-8 p-5 sm:p-7" aria-labelledby="brain-review-title">
      ...
      {items.length ? <div className="mt-5 grid gap-3">{items.map((item) => <article key={item.id} className="rounded-lg border border-zinc-800 p-4"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-mono text-xs text-zinc-500">{item.id}</p><p className="mt-1 text-xs text-zinc-500">{new Intl.DateTimeFormat("es-ES", { dateStyle: "medium", timeStyle: "short" }).format(new Date(item.created_at))}</p></div><button type="button" disabled={isPending} onClick={() => review(item.id)} className="aula-button aula-button-primary text-xs">Verificar</button></div><dl className="mt-4 grid gap-2 sm:grid-cols-3">{Object.entries(item.payload ?? {}).map(([key, value]) => <div key={key}><dt className="text-xs uppercase text-zinc-500">{key}</dt><dd className="mt-1 break-words text-sm text-zinc-200">{String(value)}</dd></div>)}</dl></article>)}</div> : <p className="mt-5 text-sm text-zinc-400">No hay evidencias pendientes.</p>}
