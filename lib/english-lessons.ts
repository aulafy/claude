...
const lmStudioFromScratchLesson: EnglishLesson = {
  courseSlug: "ia-local",
  courseTitle: "Claude Code + Local AI",
  slug: "lm-studio-desde-cero",
  title: "LM Studio from Scratch: Install and Run Local AI",
  heading: "LM Studio from Scratch: Install and Run Your First Local AI",
  description: "Install LM Studio, download a model, run AI on your computer, and verify its local API step by step on Windows, Mac, and Linux.",
  href: "/cursos/ia-local/lm-studio-desde-cero",
  blocks: [
    { type: "p", text: "LM Studio lets you download and run AI models on your own computer through a visual interface. In this lesson you will choose a model that fits, test a local chat, start the API server, and save evidence another person can reproduce. Editorial review: September 4, 2026." },
    { type: "h2", text: "What you will prove" },
    { type: "bullet", text: "The model is stored and can be loaded on your machine." },
    { type: "bullet", text: "Local chat works without a paid inference API." },
    { type: "bullet", text: "The server responds on localhost:1234." },
    { type: "bullet", text: "Another application can use an OpenAI-compatible endpoint." },
    { type: "h2", text: "LM Studio is not the model" },
    { type: "p", text: "LM Studio is the application and runtime layer. Qwen, Gemma, Mistral, Llama, and other families are models it can load. Memory use depends on the model, format, quantization, context, caches, runtime, and the rest of your system." },
    { type: "h2", text: "LM Studio or Ollama?" },
    { type: "p", text: "LM Studio makes model discovery, loading, and configuration visible in a GUI. Ollama is especially convenient for terminal workflows and automation. You do not need a universal winner: testing a similar model in both helps separate model quality from runtime experience." },
    { type: "link", text: "Ollama from Scratch", href: "/en/courses/ia-local/ollama-desde-cero" },
    { type: "h2", text: "Check compatibility before downloading" },
    { type: "bullet", text: "macOS: Apple Silicon M1-M4 and macOS 14 or newer. Intel Macs are currently unsupported." },
    { type: "bullet", text: "Windows: x64 with AVX2 or supported ARM systems. 16 GB RAM and 4 GB dedicated VRAM are current recommendations, not guarantees." },
    { type: "bullet", text: "Linux: x64 or ARM64; the desktop app is distributed as AppImage and documents Ubuntu 20.04 or newer." },
    { type: "p", text: "Start with a small instruct model, roughly 1B-8B, and a modest 4K-8K context. The goal is to verify the stack, not to force the largest model into memory." },
    { type: "h2", text: "Install LM Studio and verify lms" },
    { type: "p", text: "Download LM Studio only from the official website, open it at least once, then open a new terminal. The lms CLI ships with the application and needs no separate installation." },
    { type: "link", text: "Download LM Studio", href: "https://lmstudio.ai", external: true },
    { type: "code", text: "lms --help\nlms ls\nlms ps" },
    { type: "h2", text: "Download and load one model" },
    { type: "p", text: "In Discover, choose one chat or instruct variant. Record its size, format, quantization, and license. GGUF variants such as Q4, Q5, and Q8 trade memory against fidelity; Apple Silicon can also use supported MLX models. Do not download several variants just to complete this lesson." },
    { type: "p", text: "Load the model with conservative settings and ask: Explain local AI in one sentence. Then use lms ps to record the loaded model identifier." },
    { type: "code", text: "lms ps" },
    { type: "h2", text: "Start the local API server" },
    { type: "p", text: "Open Developer and enable Start Server, or use the CLI. The official examples use port 1234. Keep Serve on Local Network disabled for this first test." },
    { type: "code", text: "lms server start\nlms server status" },
    { type: "p", text: "Do not bind to 0.0.0.0 for this exercise. LM Studio 0.4.0 and newer can require API tokens, but localhost-only testing does not need network exposure." },
    { type: "h2", text: "Verify /v1/models" },
    { type: "code", text: "curl http://127.0.0.1:1234/v1/models" },
    { type: "p", text: "Copy the exact model id from the JSON response. Do not guess it." },
    { type: "h2", text: "Generate through the API" },
    { type: "code", text: "curl http://127.0.0.1:1234/v1/chat/completions \\\n  -H \"Content-Type: application/json\" \\\n  -d '{\n    \"model\": \"MODEL_IDENTIFIER\",\n    \"messages\": [{\"role\": \"user\", \"content\": \"Explain local AI in one sentence.\"}],\n    \"temperature\": 0.2\n  }'" },
    { type: "p", text: "If /v1/models lists the model and /v1/chat/completions returns content inside choices, you have a local inference layer another application can use." },
    { type: "h2", text: "Why use /v1 endpoints?" },
    { type: "p", text: "LM Studio supports OpenAI-compatible Models, Responses, Chat Completions, Embeddings, and other endpoints. Existing clients can often switch their base URL to http://localhost:1234/v1. Compatibility is endpoint-specific and does not make every remote-provider feature identical." },
    { type: "h2", text: "Local does not automatically mean private" },
    { type: "p", text: "After downloading and loading the model, disconnect the network temporarily and repeat a chat. A response demonstrates offline inference. Model search, downloads, updates, cloud features, web search, or remote MCP servers can still use the network. Evaluate each request's model, URL, tools, and data path." },
    { type: "h2", text: "Fast diagnosis" },
    { type: "bullet", text: "Model will not load: choose a smaller model, lighter quantization, or shorter context." },
    { type: "bullet", text: "Connection refused: check lms server status and start the HTTP server." },
    { type: "bullet", text: "Chat works but curl fails: the app can chat while the API server is stopped." },
    { type: "bullet", text: "Port 11434: that is commonly Ollama; these LM Studio examples use 1234." },
    { type: "code", text: "lms ls\nlms ps\nlms server status\nlms log stream" },
    { type: "h2", text: "Save reproducible evidence" },
    { type: "code", text: "Date and operating system:\nCPU / RAM / GPU:\nLM Studio version:\nModel / format / quantization / context:\nLocal chat: PASS / FAIL\n/v1/models: PASS / FAIL\n/v1/chat/completions: PASS / FAIL\nTime to first response:\nProblem found and next change:\nOfficial source checked:" },
    { type: "h2", text: "Official sources checked September 4, 2026" },
    { type: "link", text: "LM Studio system requirements", href: "https://lmstudio.ai/docs/app/system-requirements", external: true },
    { type: "link", text: "lms CLI", href: "https://lmstudio.ai/docs/cli", external: true },
    { type: "link", text: "Local API server", href: "https://lmstudio.ai/docs/developer/core/server", external: true },
    { type: "link", text: "API token authentication", href: "https://lmstudio.ai/docs/developer/core/authentication", external: true },
    { type: "link", text: "OpenAI-compatible endpoints", href: "https://lmstudio.ai/docs/developer/openai-compat", external: true },
    { type: "link", text: "LM Studio app privacy", href: "https://lmstudio.ai/app-privacy", external: true },
  ],
};
