...
export const dshEnglishLessons: DshLesson[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { heading: "Recommended prior route", paragraphs: ["Before using DSH with Ollama, complete the existing pages in the local AI and SME tracks. They cover the basic install, gateway setup, troubleshooting and GPU diagnosis on real machines.", "That path prevents repeated mistakes and gives you a stronger base than a standalone demo. DSH works better when the model, the network and the hardware are already understood separately."], bullets: ["Ollama from zero: `/cursos/ia-local/ollama-desde-cero`", "Connect Claude Code with local AI: `/cursos/ia-local/conectar-ollama`", "Fix Ollama errors: `/cursos/ia-local/troubleshooting-ollama`", "Ollama not using the GPU on Windows: `/cursos/ia-local/ollama-gpu-windows`", "Ollama for SMEs: safe local pilot: `/cursos/ia-pymes/ollama-piloto-seguro`"] },
